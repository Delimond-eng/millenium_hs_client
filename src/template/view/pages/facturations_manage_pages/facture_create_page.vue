                ...
                <div class="row">
                    <div class="col-12 col-md-12">
                        <div class="page-title-box d-sm-flex align-items-center justify-content-between">
                            <h4 class="mb-sm-0">Facturation patient</h4>
                        </div>
                    </div>
                </div>
                <!-- end page title -->
                    ...
                    <div class="col-md-8">
                        <div class="card">
                            <div class="card-header">
                                <h4 class="card-title mb-0 flex-grow-1 fs-12">Veuillez renseigner toutes les informations
                                    requises
                                    pour effectuer cette opération !</h4>
                            </div>
                            <div class="card-body">
                                <form @submit.prevent="submitForm" class="row ">
                                    <div class="mb-3" :class="facture_amount === '' ? 'col-md-12' : 'col-md-8'">
                                        <label for="patientCode" class="form-label fw-semibold">Type de facture
                                    ...
                                    <div class="col-md-4 mb-3" v-if="facture_amount">
                                        <label for="patientCode" class="form-label fw-semibold">Montant à
                                            payer </label>
                                        <input type="text" class="form-control fw-semibold text-success" readonly
                                    ...
                                    <div class="col-md-5">
                                        ...
                                        <div class="d-flex">
                                            <input type="number" v-model="form.paiement_montant"
                                                placeholder="Saisir le montant..." class="form-control flex-fill me-2"
                                                required>
                                            <select class="form-control" v-model="form.paiement_montant_devise"
                                                style="width:80px">
                                        ...
                                        <div class="d-flex align-items-end justify-content-end w-100 mt-4">
                                            ...
                                            <load-button btn-type="submit" :loading="formLoading"
                                                class-name="btn-success btn-border btn-label right nexttab nexttab "><i
                                                    class="ri-printer-line label-icon align-middle fs-16 ms-2 "></i>Sauvegarder
                                                & imprimer facture</load-button>
                                        </div>
                                    </div>
                                </form>
                            </div>
...
export default {
    ...
    methods: {
        ...
        submitForm(e) {
            ...
            this.$store.dispatch('services/createPaiement', this.form).then((res) => {
        ...
        this.$nextTick(() => {
            $(this.$refs.selectType).select2({
                ...
                if ($(this).select2('data')[0].row !== undefined) {
                    self.facture_amount = $(this).select2('data')[0].row.facturation_config_montant;
                    self.facture_amount_currency = $(this).select2('data')[0].row.facturation_config_montant_devise;
                }
            });
        });
    },
